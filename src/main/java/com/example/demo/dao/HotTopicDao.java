...
public interface HotTopicDao {
    @Select("select * from article,user where article.articleAuthorId = user.userId and isDel = 0  order by article.remarkNum desc limit 0,3")
    ...
    @Select("select user.userId,user.userName,user.userPhoto,a.articleNum " +
            ...
            "where a.articleAuthorId = user.userId and article.articleAuthorId = user.userId and isDel = 0 group by user.userId order by a.articleNum desc" )
